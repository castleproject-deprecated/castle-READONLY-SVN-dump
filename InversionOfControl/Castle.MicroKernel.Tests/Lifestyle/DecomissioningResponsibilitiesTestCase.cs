...
namespace Castle.MicroKernel.Tests.Lifestyle
{
	...
	public class DecomissioningResponsibilitiesTestCase
	{
		...
		[Test]
		public void ReleaseOnDispose()
		{
			kernel.AddComponent("a", typeof(A), LifestyleType.Transient);
			kernel.AddComponent("b", typeof(B), LifestyleType.Singleton);

			A itemA = kernel.Resolve<A>();
			B itemB = kernel.Resolve<B>();

			kernel.Dispose();

			// Make sure tear down doesn't crash
			kernel = new DefaultKernel();

			Assert.IsTrue(itemA.IsDisposed);
			Assert.IsTrue(itemB.IsDisposed);
		}
